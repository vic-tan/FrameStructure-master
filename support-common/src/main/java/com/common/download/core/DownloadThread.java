...
public class DownloadThread implements Runnable{
		...
		try {
			...
			if(responseCode == HttpURLConnection.HTTP_PARTIAL){
				...
				if(DownloadConfig.getInstance().getMax_download_threads() > 1){
					...
					Logger.d("DownloadThread==>" + "run()#####使用RandomAccessFile. Support ranges. Index:" + index + "==" + url + "***" + startPos + "-" + endPos + "**" + contentLength);
					...
					Logger.d("DownloadThread==>" + "run()#####使用FileOutputStream. Support ranges. Index:" + index + "==" + url + "***" + startPos + "-" + endPos + "**" + contentLength);
				...
				//不支持断点下载
				Logger.d("DownloadThread==>" + "run()#####not support ranges. Index:" + index + "==" + url + "***" + startPos + "-" + endPos + "**" + contentLength);
				...
				Logger.d("DownloadThread==>index:" + index + " run()#####server error");
			...
			e.printStackTrace();
			if(isPaused){
				Logger.d("DownloadThread==>" + " run()#####exception and pause");
				status = DownloadStatusLevel.PAUSE.value();
				listener.onDownloadPaused(index);
			}else if(isCanceled){
				Logger.d("DownloadThread==>index:" + index + " run()#####exception and cancel");
				status = DownloadStatusLevel.CANCEL.value();
				listener.onDownloadCanceled(index);
			}else{
				Logger.d("DownloadThread==>index:" + index + " run()#####error");
			...
			if(connection != null){
				...
				Logger.d("DownloadThread==>run()#####index:" + index + "***" + url + "*****close connection");
	...
	public void pause() {
		Logger.d("DownloadThread==>pause()#####index:" + index);
		isPaused = true;
		Thread.currentThread().interrupt();
	}
	
	public void cancel(){
		Logger.d("DownloadThread==>index:" + index + " cancel()");
		isCanceled = true;
		Thread.currentThread().interrupt();
	}
	...
	public void cancelByError() {
		Logger.d("DownloadThread==>index:" + index + " cancelByError()");
		isError  = true;
		Thread.currentThread().interrupt();
	}
