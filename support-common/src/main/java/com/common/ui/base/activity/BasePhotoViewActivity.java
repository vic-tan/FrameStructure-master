...
public abstract class BasePhotoViewActivity extends BaseActivity {
    ...
    protected void displayLoadingFailed(FailReason failReason) {
        if (!NetUtils.isConnected(BasePhotoViewActivity.this)) {//网络错误
            ToastUtils.show(BasePhotoViewActivity.this, R.string.common_photo_not_network);
            return;
        }
        switch (failReason.getType()) {
            case IO_ERROR://加载失败
                ToastUtils.show(BasePhotoViewActivity.this, R.string.common_photo_io_error);
                break;
            case OUT_OF_MEMORY://内存不足
                ToastUtils.show(BasePhotoViewActivity.this, R.string.common_photo_out_of_memory);
                break;
            case DECODING_ERROR://解析错误
                ToastUtils.show(BasePhotoViewActivity.this, R.string.common_photo_decoding_error);
                break;
            case UNKNOWN://未知路径
                ToastUtils.show(BasePhotoViewActivity.this, R.string.common_photo_unknown);
                break;
        }
    }
