        ...
        return new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers,
                ...
                try {
                    ...
                    if (StringUtils.isEquals(jsonBean.getCode(), JsonConstants.CODE_VALUE_0000)) {// 请求成功
                        ...
                        sendHandler(handler, RequestStatus.SUCCESS.value(), jsonBean, JsonUtils.format(replaceId(new String(responseBody))));
                    }else {// 服务错误
                        sendHandler(handler, RequestStatus.SERVICE_ERROR.value(),jsonBean, "--------------> service error (onSuccess)");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    sendHandler(handler, RequestStatus.SERVICE_ERROR.value(), new BaseJson(), Html.fromHtml("--------------> Exception (onSuccess)<br>"+e.toString()).toString());
                }
            ...
            public void onFailure(int statusCode, Header[] headers,
                ...
                if (error instanceof SocketTimeoutException) {
                    sendHandler(handler, RequestStatus.TIMEOUT_ERROR.value(),Html.fromHtml("--------------> onFailure()<br>"+error.toString()).toString());
                } else {
                    sendHandler(handler, RequestStatus.FAILURE.value(), Html.fromHtml("--------------> onFailure()<br>"+error.toString()).toString());
                }
            }
        };
    }
