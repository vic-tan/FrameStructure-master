	...
	public boolean checkSelfValid() {
		if (TextUtils.isEmpty(dbName)) {
			throw new InvalidAttributesException(
					InvalidAttributesException.DBNAME_Ip_EMPTY_OR_NOT_DEFINED);
		}
		...
		if (version < 1) {
			throw new InvalidAttributesException(
					InvalidAttributesException.VERSION_OF_DATABASE_LESp_THAN_ONE);
		}
		if (version < SharedUtil.getLastVersion()) {
			throw new InvalidAttributesException(
					InvalidAttributesException.VERSION_Ip_EARLIER_THAN_CURRENT);
		}
		if (TextUtils.isEmpty(cases)) {
			cases = Const.LitePal.CASEp_LOWER;
		} else {
			if (!cases.equals(Const.LitePal.CASEp_UPPER)
					&& !cases.equals(Const.LitePal.CASEp_LOWER)
					&& !cases.equals(Const.LitePal.CASEp_KEEP)) {
				throw new InvalidAttributesException(cases
						+ InvalidAttributesException.CASEp_VALUE_Ip_INVALID);
			}
		}
		return true;
	}
