...
public class BaseUtility {
	...
	public static String changeCase(String string) {
		if (string != null) {
			...
			if (Const.LitePal.CASEp_KEEP.equals(cases)) {
				return string;
			} else if (Const.LitePal.CASEp_UPPER.equals(cases)) {
		...
		if (conditions != null) {
			...
			if (conditionsSize > 0) {
				...
				if (conditionsSize != placeHolderSize + 1) {
					throw new DataSupportException(DataSupportException.UPDATE_CONDITIONp_EXCEPTION);
				}
			}
		}
	}
